...
long double (*Rfield)[nflds][N][N][N];//this stores the field values for each step along the grid
long double (*Rdfield)[nflds][N][N][N];//this stores the derivative of the field for each step along the grid
long double (*Sfield)[nflds][N][N][N];//this stores the field values for each step along the grid
long double (*Sdfield)[nflds][N][N][N];//this stores the derivative of the field for each step along the grid
...
int main()
{
	...
	Rfield=(long double(*)[nflds][N][N][N]) malloc(sizeof(long double)*nflds*2*N*N*N);//allocates memory for the fields 
    Rdfield=(long double(*)[nflds][N][N][N]) malloc(sizeof(long double)*nflds*2*N*N*N);//allocates memory for the fields' time derivatives
    Sfield=(long double(*)[nflds][N][N][N]) malloc(sizeof(long double)*nflds*2*N*N*N);//allocates memory for the fields
    Sdfield=(long double(*)[nflds][N][N][N]) malloc(sizeof(long double)*nflds*2*N*N*N);//allocates memory for the fields' time derivatives
    ...
    for(t=starttime;t<=endtime;t+=dt)//This is the main for-loop over time. Note that the current t value is the time for the currently evaluated fields
    {
        if(t==starttime)
        {	
            initfields();//initializes fields as defined in model.h //first time, homogeneous
			//outputslice();//outputs slice values to file //TODO new bit, check means and vars agree
			...
			initexpansion();//start expansion; //CHRIS needed to add this: typo in original GABE code
