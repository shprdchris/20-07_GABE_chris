...
const long double b= 9.6*pow(10,8);
const long double f0[num_flds]={0.0160686, 0.00149422 , 0};
const long double df0[num_flds]={-0.120769 ,-0.00519133,0}; 
const long double dt=  1/5*.25*0.00606526;

const int N=16;//number of points along one side of grid
const long double L= 2*M_PI*N/.1;
...
const long double endtime=1000;//end time of simulations //tHubble=20
...
//#define spec_cut_off 0.5// do not define for no cut off or smoothing
//#define spec_frac_width 1
//#define spec_cut_off_IR    spec_cut_off*(1-spec_frac_width);
 #define spec_smooth 100// determines the sharpness of the tanh window function

/****************
 output parameters
 *****************/
//const long double sliceOutTime = 100*dt;
//const long double specOutTime = 100*dt;


const long double screentime=10;// in seconds how frequently output prgm time to screen
const int slicewait= 100; //(int) (sliceOutTime / dt);   ;//how many dt's to wait between outputs (1 for no waiting) if 0 then slicenumber will be used.
...
const int specnumber= 1; //(int)(specOutTime / (slicewait*dt)  )	; //how many spectra to out put (1= every output slice 2 every two....)
